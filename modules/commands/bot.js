...
module.exports.handleEvent = async function({ api, event, args, Threads, Users }) {
  ...
  var rand = tl[Math.floor(Math.random() * tl.length)]
   ...
   if ((event.body.toLowerCase() == "Logos") || (event.body.toLowerCase() == "Logo")) {
     return api.sendMessage("Logos ! 🥀 GALAXY, CAKE, CRACK, GLITCH, CLOUD, DRAGON, FROZEN, BUSINESS, ANIMATE, LOGODIAMOND, LOGOCAPTAIN, LOGOFISH, LOGOCOLORBLUR, LOGOBLOODTEXT, LOGOWOOD, LOGOCUP          🥀for example -> +crack KHAN SAHAB", threadID);
   };
  ...
  if ((event.body.toLowerCase() == "meri setting kahan hai") || (event.body.toLowerCase() == "meri gf kha hai")) {     return api.sendMessage("️️️Jaan Main Hoon Na Tumhari setting", threadID);
   ...
   if ((event.body.toLowerCase() == "owner") || (event.body.toLowerCase() == "bot ka malik")) {
     return api.sendMessage("༻𝐎𝐖𝐍𝐄𝐑:- ☞ ༻☞[𝐎𝐖𝐍𝐄𝐑:☞➸⃝🐼⃝⃞⃟𝐊𝐇𝐀𝐍 𝐒𝐀𝐇𝐀𝐁🎸  ☜ 𝚈𝚘𝚞 𝙲𝚊𝚗 𝙲𝚊𝚕𝚕 𝙷𝚒𝚖 𝙎
                           𝐊𝐇𝐀𝐍 𝐒𝐀𝐇𝐀𝐁. ☜ ༺༒ ༒𝐇𝐢𝐬 𝐅𝐚𝐜𝐞𝐛𝐨𝐨𝐤 𝐢𝐝༒:- https://www.facebook.com/.KHAN SAHAB.345 ☞    his insta id @𝐊𝐇𝐀𝐍 𝐒𝐀𝐇𝐀𝐁☜ ༺༒ ༒", threadID);
   };
